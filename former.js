...
export default function addEditRemoveTask(task) {
  ...
  // add delete button when checkbox is checked
  const checkboxDelete = () => {
    const checkBoxDelete = document.createElement('button');
    checkBoxDelete.classList.add('checkbox-delete');
    checkBoxDelete.innerText = 'Delete Checked';
    const checkBoxDeleteContainer = document.querySelector('.content');
    checkBoxDeleteContainer.appendChild(checkBoxDelete);
  };
  ...
  taskCheckboxContainer.addEventListener('change', () => {
    ...
    // if (localStorage.getItem('taskStorage') === 'checked') {
    //   taskCheckboxContainer.setAttribute('checked', '');
    // }
    saveTodo();
  });
